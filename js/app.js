...
async function loadSanadChain() {
  ...
  const subtitleEn = document.querySelector('[data-sanad-subtitle-en]');
  const subtitleId = document.querySelector('[data-sanad-subtitle-id]');
  if (titleElem) titleElem.innerText = `Sanad: ${bookName} ${hadithNum}`;
  if (subtitleEn) subtitleEn.innerText = `Chain of narrators (الإسناد) for ${bookName} Hadith #${hadithNum} tracing back to the Messenger of Allah ﷺ.`;
  if (subtitleId) subtitleId.innerText = `Silsilah perawi (الإسناد) untuk ${bookName} Hadits #${hadithNum} yang bersambung sampai ke Rasulullah ﷺ.`;
  ...
  const countEnNode = document.querySelector('[data-sanad-count-en]');
  const countIdNode = document.querySelector('[data-sanad-count-id]');
  if (countEnNode) countEnNode.innerText = `${narrators.length} Narrators`;
  if (countIdNode) countIdNode.innerText = `${narrators.length} Perawi`;
