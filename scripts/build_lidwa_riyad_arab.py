...
with open('scratch/irsyadulibad_db/riyadhus-shalihin-arab.sql', 'r', encoding='utf-8') as f:
    sql = f.read()
...
try:
    ...
    c = conn.cursor()
    c.execute('SELECT * FROM riyadhus_shalihin_arab')
    rows = c.fetchall()
    
    out_dir = 'data/api/riyad_arab'
    ...
    for r in rows:
        hid = str(r[0])
        payload = {
            'id': f'riyad_arab_lidwa_{hid}',
            'book_id': 'riyad_arab',
            'dataset': 'lidwa',
            'hadith_number': hid,
            'text_ar': r[2] if len(r) > 2 else '',
            'chapter_id': 'riyad_arab_c1',
            'chapter_number': 1,
            'in_book_number': hid
        }
    ...
    print('Generated lidwa.json for riyad_arab with', len(out_data), 'hadiths.')
