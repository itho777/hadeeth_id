...
pattern = re.compile(
    r'const txt = await fetchTranslationText\(opt\);\s*if \(txt\) {\s*let output = typeof TafseerLinker !== \'undefined\' \? TafseerLinker\.parse\(txt\) : txt;\s*if \(opt\.source !== \'fawaz\' && activeDataset === \'fawazahmed\'\) \{\s*output = `<div class="mb-2 text-xs text-blue-500 font-semibold">\[Linked via Arabic matching[^>]+> \${opt\.source\.toUpperCase\(\)} #\${opt\.hid}\]</div>` \+ output;\s*\}\s*targetBox\.innerHTML = output;',
    re.DOTALL
)
...
new_js, count = pattern.subn(new_render, js)
