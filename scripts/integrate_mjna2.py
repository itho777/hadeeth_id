...
BOOKS = {
    'ibnukhuzaimah': {'name': 'Shahih Ibnu Khuzaimah', 'count': 1808, 'desc': 'Koleksi hadits shahih dari Imam Ibnu Khuzaimah.', 'icon': 'menu_book'},
    'ibnuhibban': {'name': 'Shahih Ibnu Hibban', 'count': 2769, 'desc': 'Koleksi hadits shahih dari Imam Ibnu Hibban.', 'icon': 'menu_book'},
    'mustadrak': {'name': 'Mustadrak Al-Hakim', 'count': 673, 'desc': 'Koleksi hadits dari Imam Al-Hakim An-Naisaburi.', 'icon': 'menu_book'}
}
    ...
    for book in BOOKS:
        if f"'{book}'" not in app_js:
            app_js = app_js.replace(
                ...
                f"const validBooksWithNote = ['bukhari', 'muslim', 'syafii', 'ibnukhuzaimah', 'ibnuhibban', 'mustadrak'];"
            )
            app_js = app_js.replace(
                "'syafii': 'Musnad Syafi\\'i (1,800 hadiths)'",
                f"'syafii': 'Musnad Syafi\\'i (1,800 hadiths)',\n        'ibnukhuzaimah': 'Shahih Ibnu Khuzaimah ({BOOKS['ibnukhuzaimah']['count']})',\n        'ibnuhibban': 'Shahih Ibnu Hibban ({BOOKS['ibnuhibban']['count']})',\n        'mustadrak': 'Mustadrak Al-Hakim ({BOOKS['mustadrak']['count']})'"
            )
            app_js = app_js.replace(
                "'syafii': 'Musnad Syafi\\'i (1.800 hadits)'",
                f"'syafii': 'Musnad Syafi\\'i (1.800 hadits)',\n        'ibnukhuzaimah': 'Shahih Ibnu Khuzaimah ({BOOKS['ibnukhuzaimah']['count']})',\n        'ibnuhibban': 'Shahih Ibnu Hibban ({BOOKS['ibnuhibban']['count']})',\n        'mustadrak': 'Mustadrak Al-Hakim ({BOOKS['mustadrak']['count']})'"
            )
            break
...
if html_additions:
    syafii_card = r'data-lang-id="Musnad Syafi\'i">Musnad Syafi\'i</h3>\s*</div>\s*<p class="text-sm.*?</a>'
    m = re.search(syafii_card, html, re.DOTALL)
    if m:
        html = html.replace(m.group(0), m.group(0) + '\n' + '\n'.join(html_additions))
    
    html = html.replace('<option value="syafii">Musnad Syafi\'i</option>', '<option value="syafii">Musnad Syafi\'i</option>\n            ' + '\n            '.join(dropdown_additions))
    
    open('index.html', 'w', encoding='utf-8').write(html)

print("Done integrating all books!")
