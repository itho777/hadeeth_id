    ...
    for b in books:
        ...
        for row in anchor_data:
            ...
            if os.path.exists(syarah_path):
                if os.path.getsize(syarah_path) > 300:
                    continue
                try:
                    with open(syarah_path, 'r', encoding='utf-8') as spf:
                        d = json.load(spf)
                        if d.get('syarah_ar') not in ['', 'Not Available']:
                            continue
                except:
                    pass
                print(f'[RESCUE] Re-scraping {book_id} - {anchor_id}...')
